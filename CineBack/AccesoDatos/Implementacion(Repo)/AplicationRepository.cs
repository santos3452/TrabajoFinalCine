...
namespace CineBack.Fachada.Implementacion
{
    ...
    {   
        ...
        {
            string sp = "SP_BAJA_CLIENTES";
            List<Parametro> parametro = new List<Parametro>();
            parametro.Add(new Parametro("@id", IDCliente));
            int afectadas = HelperDao.ObtenerInstancia().EjecutarSQL2(sp, parametro);
            return afectadas > 0;

        }
        public int getInsertarCliente(Clientes Cliente)
        {
            string sp = "SP_GRABAR_CLIENTE";
            ...
            return HelperDao.ObtenerInstancia().EjecutarSQL2(sp, parametros);
